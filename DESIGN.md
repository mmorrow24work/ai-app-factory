...
Each *generated* project's own `GH_PAT` (set by `scripts/factory-secrets.sh`) was already single-repo-scoped from the start — nothing changes there. The redesign only affects `ai-app-factory`'s own token.
