/**
 * Light/dark mode toggle. Ported from uk-wealth-tracker's `$lib/theme.js` (issue #116) --
 * the dark palette lives as CSS custom properties in `../app.css` (the `.dark { ... }` block);
 * this module owns applying/removing `.dark` on `<html>`, persisting the choice to
 * `localStorage`, and resolving the default from `prefers-color-scheme` before any explicit
 * choice has been made.
 *
 * SSR-safe: nothing here touches `document`/`window`/`localStorage` at import time, since
 * `site/` prerenders every route (adapter-static). {@link theme} therefore starts at a fixed
 * default on both server and client -- call {@link refreshTheme} from `onMount` to read the real
 * value in and apply it.
 *
...
export const THEME_STORAGE_KEY = 'ai-app-factory:theme';
 ...
 * system preference. There is no third "system" mode stored; once the user picks light or dark
 * via {@link setTheme}, that choice sticks even if their OS preference later changes.
 *
 * @returns {Theme}
 */
 ...
 *
 * @param {Theme} value
 * @returns {Theme} `value`, unchanged -- returned so callers can chain it.
 */
